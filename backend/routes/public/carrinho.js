...
routerCarrinho.get('/:id_utilizador', (req, res) => {
    ...
    db.query('SELECT * FROM carrinhos WHERE id_utilizador = ? AND estado = "ativo" LIMIT 1', [id_utilizador], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            db.query(
                'SELECT items_carrinhos.*, produtos.nome, produtos.imagem_url FROM items_carrinhos JOIN produtos ON items_carrinhos.id_produto = produtos.id WHERE items_carrinhos.id_carrinho = ?',
                [carrinho.id],
                (err, items) => {
